...
export default function ContentCourse() {
  ...
  const door = doors.map((item, index) => (
    ...
    >
      ...
      <Text fw={500} fz={15}>{item.title}</Text>
      <Box>
        <Box c={""}>{item.icon}</Box>
      </Box>
    ...
    <Box  className={classes.parent}>
      ...
      <Box py={15} px={50}>
        ...
        <Box  mb={20} className={classes.containerTabs}>
          ...
          <Box mt={30} px={20} pt={15} pb={15} bg={"rgba(34, 166, 241, 0.1)"} style={{borderRadius:"10px"}} className={classes.teacherData}>
            <Text ta={"center"} mb={10} fz={15} fw={700}>عن المعلم</Text>
        ...
        <Box mb={20} ta={"center"}>{actBtn}</Box>
      </Box>
    </Box>
  );
}
